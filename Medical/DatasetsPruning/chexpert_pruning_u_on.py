...
def preprocess_and_filter_chexpert(mode='train'):
    ...
    # Xử lý nhãn không chắc chắn (U-On Strategy: thay -1.0 bằng 0.5)
    print("Processing uncertain labels (-1.0 to 0.5)...")
    for column in FINAL_LABEL_SET:
        df_filtered[column] = df_filtered[column].apply(lambda x: 0.5 if x == -1.0 else x)
    ...
    # Update the 'Path' column to the new relative paths
